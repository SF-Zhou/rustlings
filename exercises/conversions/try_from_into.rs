...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple.0 < 0 || tuple.0 > 255 {
            return Err(format!("color {} is invalid", tuple.0));
        }
        if tuple.1 < 0 || tuple.1 > 255 {
            return Err(format!("color {} is invalid", tuple.1));
        }
        if tuple.2 < 0 || tuple.2 > 255 {
            return Err(format!("color {} is invalid", tuple.2));
        }
        Ok(Color {
            red: tuple.0 as u8,
            green: tuple.1 as u8,
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            return Err(format!("slice length {} != 3", slice.len()));
        }
        for &color in slice {
            if color < 0 || color > 255 {
                return Err(format!("color {} is invalid", color));
            }
        }
        Ok(Color {
            red: slice[0] as u8,
